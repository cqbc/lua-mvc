...
if not clib then
    ngx.log(ngx.ERR, 'can not load libredis_slot.so')
end
    ...
    for i=1,#serv_list do
        ...
        if ok then
            ...
            if slot_info then
                ...
                ngx.log(ngx.ERR, 'red:slot_info() error, cause: ' .. tostring(err))
            end
        else
            ngx.log(ngx.ERR, 'red:connect() error, cause: ' .. tostring(err) .. ', ipString: ' .. ip_string(ip) .. ':' .. tostring(port))
        end
    end
end
